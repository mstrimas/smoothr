...
circles_poly <- map(circles, as.polygons) |>
  map(~ select(st_as_sf(.x), geometry)) |>
  bind_rows() |>
...
ply_geom <- st_polygon(list(xy)) |>
  st_sfc(crs = 4326)
ply <- st_sf(geometry = ply_geom) |>
  mutate(type = "polygon", hole = FALSE, multipart = FALSE)
...
mp2 <- as.polygons(r_mp2) |>
  ...
  select(geometry) |>
  mutate(type = "polygon", hole = FALSE, multipart = TRUE)
mp3_geom <- rbind(chole_poly[2, ], circles_poly[2, ]) |>
  st_combine()
mp3 <- st_sf(geometry = mp3_geom) |>
...
open_lines_geom <- st_sfc(list(l1, l2, l3), crs = 4326)
open_lines <- st_sf(geometry = open_lines_geom) |>
...
j3d1_geom <- st_linestring(
  cbind(cos(theta), sin(theta), theta / pi / 2),
  dim = "XYZ"
) |>
  st_sfc(crs = 4326)
j3d1 <- st_sf(
...
j3d2_geom <- st_linestring(
  cbind(cos(theta), sin(theta) / 4, sin(2 * theta)) / 4,
  dim = "XYZ"
) |>
  st_sfc(crs = 4326)
j3d2 <- st_sf(
...
gaussian_field <- function(r, range, beta = c(1, 0, 0)) {
  ...
  preds <- predict(gsim, newdata = coords, nsim = 1)
  vals <- preds$sim1
